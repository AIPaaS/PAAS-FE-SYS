					...
					<table class="table">
						<thead>
							<tr>
								...
								<th class="text-center" id="th_assrole">分配角色</th>
							</tr>
						</thead>
						<tbody id="tabList">
...
{{each(i,row) data}}
		<tr>
			<td class="text-center">{{= row.op.opCode}}</td>
			<td class="text-center">{{= row.op.opName}}</td>
			<td class="text-center">{{= row.op.emailAdress}}</td>
			<td class="text-center">{{html getRoleNames(row.roleIds)}}</td>
			<td class="text-center">
				<a id="a_assign_role_{{= row.op.id}}" href="###" class="table-link danger" title="分配角色" data-type="checklist" data-pk="{{= row.op.id}}" data-value="{{= row.roleIds}}" data-title="分配角色">
					<span class="fa-stack">

...
function query(pageNum){
	...
	RS.ajax({url:"/mnt/user/queryOpPage",ps:ps,cb:function(r) {
		if(!CU.isEmpty(r)){
			...
			for(var i=0;i<data.length;i++){
				$("#a_remove_"+data[i].id).bind("click", function() {
...
function removeOp(obj) {
	CC.showMsg({msg:"您确定要<font color='red'>注销</font>用户<font color='blue'>["+obj.opCode+"]"+obj.opName+"</font>吗?",option:2,callback:function(cr) {
		if(cr != "ok") return ;
		RS.ajax({url:"/mnt/user/removeOpById",ps:{opId:obj.id},cb:function() {
			query();
		}});
	}});
}
